...
LX9quad_p1 <- function(gene_file,meta_file,group1,group2){
   ...
   R_packs_install <- function(){
    ...
    if(length(new_R_packs)!=0){install.packages(new_R_packs,quietly = TRUE)
...
#  write.xlsx(group,"temporary files/group.xlsx")
  ...
  table(duplicated(gene_all$gene_id)) # check the duplicated genes
  gene_all <- limma::avereps(gene_all[,-1],ID= gene_all$gene_id) %>% data.frame() # 对重复的gene_id,取其平均值，同时也有去重功能
  gene_all$gene_id <- rownames(gene_all)
  gene_all <- gene_all[,c(8,1:7)]
  rownames(gene_all) <- 1:nrow(gene_all)

  meta_all <- read.xlsx(meta_file)
  meta_all$sum <- rowSums(meta_all[,4:9])
  meta_all <- dplyr::filter(meta_all,sum>0)
  meta_all <- meta_all[,-10]
  colnames(meta_all)[1:2]=c("meta_id","MaxFC")
  ...
  meta_data <- meta_all[,-2:-3]
  table(duplicated(meta_data$meta_id)) # check the duplicated metabolites
  meta_data <- limma::avereps(meta_data[,-1],ID= meta_data$meta_id) %>% data.frame() # 对重复的meta_id,取其平均值，同时也有去重功能
  meta <- t(meta_data)

  meta_n <- ncol(meta)

  meta_FC_data <- meta_all[,1:3]
  colnames(meta_FC_data) <- c("meta_id","MaxFC","group")
  meta_FC_data$FC <- NA
  #meta_FC_data[,2] <- as.numeric(meta_FC_data[,2])
  ...
  for(i in 1:meta_n){
    if(tolower(trimws(meta_FC_data[i,3]))==tolower(group2))
      meta_FC_data[i,4] <- 1/meta_FC_data[i,2]
    else
      meta_FC_data[i,4] <- meta_FC_data[i,2]
  }

  meta_FC <- meta_FC_data[,c(-2,-3)]
  table(duplicated(meta_FC$meta_id)) # check the duplicated metabolites
  meta_FC <- limma::avereps(meta_FC[,-1],ID= meta_FC$meta_id) %>% data.frame() # 对重复的meta_id,取其平均值，同时也有去重功能
  meta_FC$meta_id <- rownames(meta_FC)
  rownames(meta_FC) <- 1:nrow(meta_FC)
  meta_FC <- meta_FC[,c(2,1)]
  meta_FC$log2FC <- log2(meta_FC[,2])
  meta_FC <- meta_FC[,-2]
  colnames(meta_FC) <- c("meta_id","log2FC")
