...
group1_n <- grep(group_meta[1],names(meta_df),ignore.case = T)
group2_n <- grep(group_meta[2],names(meta_df),ignore.case = T)


FC_df <- meta_df

FC_df[grepl("inf",FC_df$FC,ignore.case = T),2] <- 100
FC_df$FC <- as.numeric(FC_df$FC)

FC_df$Highest.Mean <- ifelse(rowSums(FC_df[group2_n])/rowSums(FC_df[group1_n])>1, group_meta[2], group_meta[1])
FC_df$FC <- ifelse(rowSums(FC_df[group2_n])/rowSums(FC_df[group1_n])>1, log2(FC_df$FC), log2(1/FC_df$FC))
...
#meta_FC_data[grepl("inf",meta_FC_data$FC,ignore.case = T),2] <- 100
#meta_FC_data[grepl(group1,meta_FC_data$group,ignore.case = T),4] <- log2(meta_FC_data[grepl(group1,meta_FC_data$group,ignore.case = T),2])
#meta_FC_data[grepl(group2,meta_FC_data$group,ignore.case = T),4] <- log2(1/meta_FC_data[grepl(group2,meta_FC_data$group,ignore.case = T),2])
#meta_FC <- meta_FC_data[,c(1,4)]
...
titile_text <- paste('Nine Quadrantal Diagram',"(",group_meta[2],"VS",group_meta[1],")")
...
LX9quad_p2 <- function(xmin,xmax,ymin,ymax,mycolor){
